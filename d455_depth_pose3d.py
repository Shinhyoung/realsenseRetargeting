...
try:
    while True:
        ...
        # ── Depth → JET 컬러맵 (MediaPipe 입력으로 사용) ──
        depth_colormap = cv2.applyColorMap(
            cv2.convertScaleAbs(depth_image, alpha=0.03),
            cv2.COLORMAP_JET,
        )

        # ── MediaPipe Pose 추론 (Depth 컬러맵 입력) ──
        depth_rgb = cv2.cvtColor(depth_colormap, cv2.COLOR_BGR2RGB)
        mp_image = mp.Image(image_format=mp.ImageFormat.SRGB, data=depth_rgb)
        ...
        if results.pose_landmarks:
            ...
            for name, lm_id in LANDMARKS.items():
                ...
                cv2.putText(
                    display, coord_text, (10, y_offset),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1,
                )
                y_offset += 22

        cv2.imshow("D455 Depth Pose 3D | Press 'q' to quit", display)
